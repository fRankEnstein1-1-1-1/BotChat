...
// Define a type for your chat history if you're using TypeScript,
// aligning it with Gemini's format:
// { role: "user" | "model", parts: { text: string }[] }
interface GeminiChatHistoryItem {
    role: "user" | "model"; // This 'role' is for the Gemini API's expected format
    parts: { text: string }[];
}

export async function generateChatCompletion(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const history: GeminiChatHistoryItem[] = user.chats.map((chatItem) => ({
